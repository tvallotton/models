...
impl<'a> Parser<'a> {
    ...
    pub fn parse_prefix(&mut self) -> Result<Expr, ParserError> {
        ...
        //      BOOL 'true'
        //
        // The first two are standard SQL, while the latter is a PostgreSQL extension.
        // Complicating matters is the fact that INTERVAL string literals may
        // optionally be followed by special keywords, e.g.:
        //
        //      INTERVAL '7' DAY
        //
        // Note also that naively `SELECT date` looks like a syntax error because the
        // `date` type name is not followed by a string literal, but in fact in
        // PostgreSQL it is a valid expression that should parse as the column
        // name "date".
    ...
    /// Parse `CURRENT ROW` or `{ <positive number> | UNBOUNDED } { PRECEDING |
    /// FOLLOWING }`
    ...
    /// Parse a SQL LISTAGG expression, e.g. `LISTAGG(...) WITHIN GROUP (ORDER
    /// BY ...)`.
    pub fn parse_listagg_expr(&mut self) -> Result<Expr, ParserError> {
        self.expect_token(&Token::LParen)?;
        let distinct = self.parse_all_or_distinct()?;
        let expr = Box::new(self.parse_expr()?);
        // While ANSI SQL would would require the separator, Redshift makes this
        // optional. Here we choose to make the separator optional as this
        // provides the more general implementation.
        ...
        self.expect_token(&Token::RParen)?;
        // Once again ANSI SQL requires WITHIN GROUP, but Redshift does not. Again we
        // choose the more general implementation.
    ...
    pub fn parse_literal_interval(&mut self) -> Result<Expr, ParserError> {
        ...
        let (leading_precision, last_field, fsec_precision) =
            if leading_field == Some(DateTimeField::Second) {
                ...
                // Instead of
                //     `SECOND [(<leading precision>)] TO SECOND[(<fractional seconds
                // precision>)]` one must use the special format:
    ...
    /// Parse a postgresql casting style which is in the form of
    /// `expr::datatype`
    ...
    /// Bail out if the current token is not an expected keyword, or consume it
    /// if it is
    ...
    /// Parse either `ALL` or `DISTINCT`. Returns `true` if `DISTINCT` is parsed
    ...
    /// keyword) Some examples with aliases: `SELECT 1 foo`, `SELECT
    /// COUNT(*) AS cnt`, `SELECT ... FROM t1 foo, t2 bar`, `SELECT ... FROM
    /// (...) AS bar`
    ...
    pub fn parse_table_factor(&mut self) -> Result<TableFactor, ParserError> {
        if self.parse_keyword(Keyword::LATERAL) {
            ...
            // A parsing error from `parse_derived_table_factor` indicates that the '('
            // we've recently consumed does not start a derived table (cases 1,
            // 2, or 4). `maybe_parse` will ignore such an error and rewind to
            // be after the opening '('.
